...
1. `POST /admin/bindings/{binding_id}/impact-preview` writes a content-free `audit_event` whose
   ...
   logical KB id, live config version, and runtime flags. Confirming a different binding,
   operation, version, or already-consumed preview fails closed.
...
3. Disable and kill switch update independent runtime flags. Retiring the final retrieval-enabled
   binding also transitions its logical KB to `retired`; other safe bindings keep the KB available.

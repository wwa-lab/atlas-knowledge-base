...
1. `POST /admin/bindings/{binding_id}/impact-preview` writes a content-free `audit_event` whose
   ...
   state change share one transaction, so failed validation does not burn a valid preview. Retire
   previews additionally persist a content-free sibling-state fingerprint generated by the same
   lifecycle-appropriate eligibility predicate used for the final retirement decision. Retire locks
   the KB row and its binding rows in a stable order before validating that fingerprint; binding
   and lifecycle writers use the same row-locking boundary through the repositories.

...
class GovernanceApiTest {
    ...
    @Test
    void suspendedRetirePreviewStalesWhenSafeSiblingChanges() throws Exception {
        LoggedIn owner = login("kb_owner");
        String bindingId = activeDifyBinding(owner, "Governance suspended stale " + UUID.randomUUID());
        String logicalKbId =
                jdbcTemplate.queryForObject(
                        "SELECT logical_kb_id FROM binding WHERE binding_id = ?", String.class, bindingId);
        String remainingBindingId = "bnd_suspended_stale_" + UUID.randomUUID();
        Instant now = Instant.now();
        jdbcTemplate.update(
                """
                INSERT INTO binding (
        ...
        String preview = preview(admin, bindingId, "{\"operation\":\"retire\"}");
        jdbcTemplate.update(
                "UPDATE binding SET kill_switch = 1, config_version = config_version + 1 WHERE binding_id = ?",
                remainingBindingId);
                        ...
                        post("/api/v1/admin/bindings/" + bindingId + "/retire")
                                ...
                                .content(
                                        "{\"confirm\":true,\"impact_preview_id\":\""
                                                ...
                                                + "\"}"))
                .andExpect(status().isConflict())
                .andExpect(jsonPath("$.error.code").value("IMPACT_PREVIEW_STALE"));
        assertThat(
                        jdbcTemplate.queryForObject(
                                "SELECT lifecycle FROM logical_knowledge_base WHERE logical_kb_id = ?",
                                String.class,
